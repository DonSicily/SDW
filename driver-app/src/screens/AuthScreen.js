...
export default function AuthScreen({ setIsAuthenticated, isDriver = true }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!isLogin && isDriver && !vehicleId) {
      Alert.alert('Error', 'Please enter your vehicle ID');
      return;
    }
    if (!isLogin && !otpCode) {
    ...
    try {
      if (isLogin) {
        ...
        const res = await API.post('/auth/register', {
          ...
          otpCode,
          vehicleDetails: isDriver ? { vehicleId } : undefined
        });
        if (res.data.token || res.data.accessToken) {
          await persistSession(res.data);
        } else {
  ...
  return (
    <View style={styles.container}>
      <Image
        ...
        resizeMode="contain"
      />
      <Text style={styles.title}>{isDriver ? 'SDW Taxi Driver' : 'SDW Taxi Rider'}</Text>
